// Games screen: shared page chrome (lamp/h1/sub) for the four recurring, lesson-independent
// drills — Numbers 1-999, Date & Weekday, Mahjong Tiles, and Colors — picked with the same
// mode-toggle pattern the Lessons screen uses for Flip/Type/Listen/Build. Each game owns
// everything below the toggle (autoplay row, speed row, its own round/session logic) and mounts
// into #gameArea exactly as it would mount into a full screen — this module never reaches into a
// game's internals.
const Games = (() => {
  let root = null;
  let goTo = null;
  let game = 'numbers'; // 'numbers' | 'dateWeekday' | 'mahjong' | 'colors'
  ...
  function buildToggle() {
    ...
    el.innerHTML = `
      ...
      <div class="mode-btn ${game === 'colors' ? 'active' : ''}" id="colorsGameBtn">Colors</div>
    `;
    ...
    root.querySelector('#colorsGameBtn').onclick = () => switchGame('colors');
  }
  ...
  function renderGame() {
    ...
    else if (game === 'mahjong') MahjongGame.mount(area, goTo);
    else ColorsGame.mount(area, goTo);
  }
